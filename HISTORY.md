...
## v0.12.1 — メンテナンス期 第 2 巡（ホストなし DSN の露出とパネル寸法）
**PR**: #114, #115
**実装**: v0.12.0 以降のバグ修正 2 件。機能追加なし。

**認証情報の露出（#114, Issue #113）**: ホストを持たない DSN（`postgres://alice:secret@/db` など）では `DisplayName` が生の DSN にフォールバックし、パスワードがステータスバーと compare ペインのラベルに出ていた。pgx は Unix ソケット、go-sql-driver は 127.0.0.1 で実際に接続できるため、壊れた DSN だけの経路ではない。フォールバックを `MaskDSN(dsn)` にした（`localhost` 表示はソケット経由であることを隠すため採らない）。

**UI の寸法（#115）**: lipgloss v1 の `Width`/`Height` は枠線を含まないため、枠付きパネルが割り当てより 2 セル大きく描かれ、結果パネルの右・下の枠が常に欠け、compare の右ペインが画面外にはみ出していた。枠込みで w×h に収める `framedPanel` を導入し、サイドバー幅・ステータスバー（常に 1 行、ヒントから先に `…` で切る）・高さ計算（JoinVertical 分として引いていた 2 行を戻す）を合わせて直した。`TestViewFitsTerminal` / `TestStatusBarStaysOneRow` で端末サイズちょうどに収まることを固定。

---

## v0.12.0 — メンテナンス期 第 1 巡（レビュー指摘の解消と readonly 層2 の拡張）
**PR**: #60〜#64, #69, #73〜#78, #92〜#103, #106〜#112
**実装**: v0.11.0 以降、メンテナンス期に入って 3 回の監査・全体レビュー（2026-08-30 / 08-31 / 09-07）で再現した指摘を 1 件 1 PR で解消した。機能追加は readonly 層2 の拡張だけ。

**2026-08-30 の監査（#60〜#64, #69, #73〜#78）**: JSON エクスポートの重複キー除去で元ヘッダを保持（#60）、MySQL の DSN を文字列連結ではなく driver の `Config` で組み立て（#61）、接続切替後の古いテーブル一覧ロードを破棄（#62）、数値に見える BLOB を数値昇格の前に BLOB と分類（#63）、`govulncheck` を PR・main・週次で CI 実行（#64）。Stats overlay と各 overlay の幅計算をルーン数ではなくセル幅で行い（#69, #73, #74, #75）、サイドバーのテーブル名をサニタイズ（#77）、active な bring DB への bring 後にテーブル一覧を再読込（#76）、Windows コンソールで Ctrl+Enter が効かない旨を README に明記（#78）。

**readonly 層2 を MySQL / PostgreSQL に拡張（#103）**: Docker の実サーバ（`mysql:8.4` / `postgres:17`）で「DSN パラメータがプール内の全コネクションに効く」ことを計測してから入れた。MySQL は `transaction_read_only=1`、PostgreSQL は `default_transaction_read_only=on`。ただし SQLite の `mode=ro` と違い、どちらもセッション自身が `SET` で外せるので**強度は揃っていない**。外す文を拒否するのは層1 で、層1 が防御の本体であることは変わらない。変数を知らないサーバ（MariaDB 11.1 未満・プーラー配下）ではパラメータなしで再接続する。統合テストは `testdata/compose.yaml` + `//go:build integration`（`go.mod` は不変）。

**認証情報の露出（#93, #106）**: `mysql://` / `postgres://` の URL パースエラーが `*url.Error` を `%w` で保持し、パスワードを stderr と TUI に出していた。`db.URLParseError` に集約して元エラーを包まない形にし、`MaskDSN` の malformed 経路も監査して 3 件（パスワード中の `/`、短すぎるマッチ、大文字の `?PASSWORD=`）を塞いだ。過剰マスク側に倒したため #93 の期待値を 1 件反転している。正常系のホストなし DSN の漏洩は #113 に分離。

**UI の正確性（#92, #95〜#102, #107〜#111）**: スニペット/プロファイル削除時のリストコピー（#92）、補完 prefix を表示幅ではなくルーン列で索引（#95）、キャンセル時の `querySeq` bump（#96）、`?` を含む SQLite パス（#97）、`time.Time` のサブ秒精度と時刻対応の比較（#98）、viewport の rebuild-skip cache 復活（#99）、compare の paneHeight クランプ（#100）、補完ポップアップのオーバーレイ化（#101）、接続切替の世代ガードと `Prepare`/`Activate` 分離（#102）。2026-09-07 の巡では、ヒストグラムの bin 添字を `[0,1]` 正規化から求めて amd64 の panic を止め（#107。arm64 では panic せず 1 bin に潰れるため、テストは bin ごとの件数を assert する）、compare ペイン切替で table の Focus も移し（#108）、ペインのラベルを結果の出所 `lastConn` に（#109）、全テーブル列補完を 1 Cmd の一括取得にしてキャッシュ上限は補完後に効かせ（#110。65 テーブル以上で取得ループが止まらなかった）、2^53 超の整数を厳密比較（#111。float64 で同点のときだけ `big.Rat`）。

**SQL 分類（#94, #112）**: MySQL の `Schema()` がビューで失敗する問題（#94）。`LeadingKeyword` の境界を空白から識別子の終わりに変え、`SELECT(1)` / `VALUES(1)` の結果が Exec 経路で捨てられる問題を修正（#112）。readonly の分類表への影響は「先頭識別子が許可キーワードで直後に非識別子文字が来る文」が新たに許可されるだけで、新たに拒否されるものはない。
